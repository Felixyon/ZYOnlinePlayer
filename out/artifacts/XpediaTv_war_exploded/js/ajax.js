...
function ajaxSearch() {
    ...
    $.ajax({
        ...
        url: mkPlayer.api, 
        data: "types=search&id=" + rem.wd,
        dataType : "json",
        complete: function(XMLHttpRequest, textStatus) {
            if(tmpLoading) layer.close(tmpLoading);    // 关闭加载中动画
        },  // complete
        success: function(jsontext){
            var jsonData=jsontext.result.songs;
            // 调试信息输出
            if(mkPlayer.debug) {
            ...
            for (var i = 0; i < jsonData.length; i++) {
                ...
                tempItem =  {
                    ...
                    artist: jsonData[i].ar[0].name, // 艺术家名字
                    album: jsonData[i].al.name,    // 专辑名字
                    source: '网易云音乐',     // 音乐来源
                    url_id: jsonData[i].al.id,  // 链接ID
                    pic_id: jsonData[i].id,  // 封面ID
                    lyric_id: jsonData[i].id,  // 歌词ID
                    pic: jsonData[i].al.picUrl,    // 专辑图片
                    url: null   // mp3链接
                };
...
function ajaxUrl(music, callback)
{
    ...
    $.ajax({ 
        type: mkPlayer.method, 
        url: mkPlayer.api,
        data: "types=url&id=" + music.id,
        dataType : "json",
        success: function(jsontext){
            if (jsontext.code===200) {
                var jsonData=jsontext.data[0];
            }else {
...
function ajaxPic(music, callback)
{
    ...
    $.ajax({ 
        type: mkPlayer.method, 
        url: mkPlayer.api,
        data: "types=pic&id=" + music.pic_id,
        dataType : "json",
        success: function(jsontext){
            var jsonData=jsontext.result.songs[0].al;
            // 调试信息输出
            if(mkPlayer.debug) {
                console.log("歌曲封面：" + jsonData.picUrl);
            }
            
            if(jsonData.picUrl !== "") {
                music.pic = jsonData.picUrl;    // 记录结果
            } else {
                music.pic = "err";
            }
...
function ajaxLyric(music, callback) {
    ...
    $.ajax({
        type: mkPlayer.method,
        url: mkPlayer.api,
        data: "types=lyric&id=" + music.lyric_id,
        dataType : "json",
        success: function(jsontext){
            var jsonData=jsontext.lrc;
            // 调试信息输出
            if (mkPlayer.debug) {
